...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    label { display: grid; gap: 5px; color: #637083; font-size: 12px; font-weight: 650; }
    input { min-height: 38px; border: 1px solid #ccd5e1; border-radius: 6px; padding: 8px 10px; font: inherit; }
    ...
    .metric, section { background: #fff; border: 1px solid #d8dee8; border-radius: 8px; }
    .metric { padding: 14px; }
    ...
    @media (max-width: 560px) { main { padding: 14px; } form, .grid { grid-template-columns: 1fr; } }
  </style>
</head>
<body>
  ...
  <script>
    ...
    async function poll() {
      ...
      if (status.status === 'completed') await loadResults();
    }
    async function loadResults() {
      ...
      document.getElementById('dash').innerHTML = `<a class="button" href="/api/scans/${scanId}/dashboard" target="_blank" rel="noopener">Open graph dashboard</a>`;
